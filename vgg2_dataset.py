...
class Vgg2Dataset:
    def __init__(self, partition='train', imagesdir='vggface2_data/<part>', csvmeta='vggface2/<part>.detected1.csv', target_shape=(224,224,3), augment=True, custom_augmentation=None, preprocessing='full_normalization', debug_max_num_samples=None):
        ...
        cache_file_name = '%s%s.cache'%(csvmeta.replace('/','_').replace('<part>','part'), '.'+str(debug_max_num_samples) if debug_max_num_samples is not None else '')
        try:
    ...
    while True:
        ...
        for batch in tqdm(gen):
            for im, identity in zip(batch[0], batch[1]):
                ...
                cv2.putText(im, "%d %s"%(identity, get_vgg2_identity(identity)[0]), (0,im.shape[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255))
