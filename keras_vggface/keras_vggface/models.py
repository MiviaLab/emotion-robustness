...
def VGG16(include_top=True, weights='vggface',
    ...
    print("Weights before loading for layer", model.layers[12].name)
    print(model.layers[12].get_weights()[0][0,0,0,0:5])
    model.summary()
    if weights == 'imagenet':
        if include_top and classes==1000:
            weights_path = get_file('vgg16_imagenet_tf_vgg16.h5',
                                    utils.VGG16_IMAGENET_WEIGHTS_PATH,
                                    cache_subdir=utils.VGGFACE_DIR)
            model.load_weights(weights_path)
        else:
            weights_path = get_file('vgg16_imagenet_tf_notop_vgg16.h5',
                                    utils.VGG16_IMAGENET_WEIGHTS_PATH_NO_TOP,
                                    cache_subdir=utils.VGGFACE_DIR)
            top_layers = list()
            if include_top:
                for i in range(6):
                    top_layers.append(model.layers.pop())
            model.load_weights(weights_path)
            if include_top:
                for i in range(6):
                    model.layers.append(top_layers.pop())
        #try:
        #except ValueError as e:
        #    print(e)
        print("Weights after loading for layer", model.layers[12].name)
        print(model.layers[12].get_weights()[0][0,0,0,0:5])
