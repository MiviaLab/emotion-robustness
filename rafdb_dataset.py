...
def _load_rafdb(meta, imagesdir, partition):
    ...
    for n,d in enumerate(tqdm(meta)):
        ...
        if actualpartition == partition:
            ...
            if not drop:
                path = os.path.join(imagesdir,'%s_%s_aligned.jpg'%(partition,imgid))
                img = cv2.imread(path)
                if img is not None:
                    example={
                        'img': img,
...
class RAFDBDataset:
    def __init__(self, partition='train', imagesdir='RAF-DB/basic/Image/aligned', csvmeta='RAF-DB/distribute_basic.csv', target_shape=(224,224,3), augment=True, custom_augmentation=None, preprocessing='full_normalization', debug_max_num_samples=None):
        ...
        cache_file_name = '%s.%s%s.cache'%(imagesdir.replace('/','_'),partition, '.'+str(debug_max_num_samples) if debug_max_num_samples is not None else '')
        try:
